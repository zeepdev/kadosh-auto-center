...
- **Gastos Fixos & Parcela Mãe**: Gastos contínuos ou parcelados com término devem ser agrupados pelo `parent_id` sob um registro mestre (`is_parent: true`). A visualização na tabela deve ser mantida compacta na Parcela Mãe, com desdobramento em sanfona/accordion (`expandedParents`) ao clicar para exibir todas as parcelas filhas com seus respectivos vencimentos, status e ações individuais de pagamento.
